...
* `WizardMatchHudLayout` owns only narrow responsive behavior such as contextual inspector placement, sidebar clamping, and z-order. Static piles, hand trays, portraits, public-zone trays, and turn-panel placement remain authored directly in `local_wizard_match_screen.tscn` so their table locations are visible and adjustable in the editor.
...
* `HudLayer` is now a `Control`, not a plain `Node`, so authored HUD anchors resolve against a proper control root at runtime. This is part of the current UI architecture, not an implementation detail to undo.
