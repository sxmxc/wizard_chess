...
func load_fen(fen: String) -> void:
	...
	for rank_index in range(8):
		...
		for token in rank_rows[rank_index]:
			...
			board[rank_index][file_index] = _make_piece(piece_type, color)
			file_index += 1
...
func to_fen(include_counters: bool = true) -> String:
	...
	var fen := "%s %s %s %s" % [
		"/".join(rows),
		"w" if active_color == WHITE else "b",
		castling,
		"-" if en_passant_target == null else square_to_algebraic(en_passant_target),
	]

	if include_counters:
		fen += " %d %d" % [halfmove_clock, fullmove_number]
...
func get_legal_moves_from(square: Vector2i) -> Array:
	...
	if piece == null or piece["color"] != active_color or outcome["status"] != STATUS_ACTIVE:
		return []
...
func claim_draw() -> bool:
	if claimable_draw_reason.is_empty() or outcome["status"] != STATUS_ACTIVE:
		return false
...
func _generate_pawn_moves(square: Vector2i, piece: Dictionary) -> Array:
	...
	for file_offset in [-1, 1]:
		...
		if en_passant_target != null and target == en_passant_target:
...
func _generate_king_moves(square: Vector2i, piece: Dictionary) -> Array:
	...
	var rights = castling_rights[piece["color"]]
...
func _apply_move_unchecked(move: Dictionary, record_history: bool, refresh_state: bool = true) -> void:
	...
	var turn_number := fullmove_number
	...
	if move["is_en_passant"]:
		...
		if get_piece(captured_square) != null:
			board[captured_square.y][captured_square.x] = null
...
func _refresh_outcome() -> void:
	...
	var legal_moves := get_legal_moves_for_color(active_color)
	if legal_moves.is_empty():
		if is_in_check(active_color):
			outcome = {
				"status": STATUS_CHECKMATE,
				"winner": _opponent(active_color),
	...
	var current_position_count: int = position_history.get(_position_key(), 0)
	if current_position_count >= 3:
		claimable_draw_reason = DRAW_THREEFOLD_REPETITION
	elif halfmove_clock >= 100:
		claimable_draw_reason = DRAW_FIFTY_MOVE_RULE


func _record_position() -> void:
	var key := _position_key()
	position_history[key] = int(position_history.get(key, 0)) + 1
...
func _notation_suffix() -> String:
	if outcome["status"] == STATUS_CHECKMATE:
		return "#"
	if outcome["status"] == STATUS_ACTIVE and is_in_check(active_color):
		return "+"
	return ""
