...
const HOVERED_SQUARE_TINT := Color("f0d78a")
...
var white_portrait_texture: Texture2D
var black_portrait_texture: Texture2D
...
func _resolve_ui_node(candidates: Array[String]) -> Node:
	for candidate in candidates:
		var node := get_node_or_null(candidate)
		if node != null:
			return node
		var leaf_name := candidate.get_file()
		if not leaf_name.is_empty():
			node = find_child(leaf_name, true, false)
			if node != null:
				return node
	return null
...
@onready var turn_action_panel := _resolve_ui_node(["HudLayer/TurnPanel", "%TurnPanel", "TurnPanel"]) as WizardMatchTurnActionPanel
...
@onready var notification_layer := _resolve_ui_node(["NotificationLayer", "%NotificationLayer"]) as CanvasLayer
@onready var targeting_overlay: TargetingOverlay = %TargetingOverlay
@onready var opponent_hand_panel := _resolve_ui_node(["HudLayer/OpponentHandPanel", "%OpponentHandPanel", "OpponentHandPanel"]) as Control
@onready var local_hand_panel := _resolve_ui_node(["HudLayer/LocalHandPanel", "%LocalHandPanel", "LocalHandPanel"]) as Control
@onready var opponent_deck_pile := _resolve_ui_node(["HudLayer/OpponentLibraryPanel", "%OpponentLibraryPanel", "OpponentLibraryPanel"]) as WizardMatchPileView
@onready var opponent_graveyard_pile := _resolve_ui_node(["HudLayer/OpponentGraveyardPanel", "%OpponentGraveyardPanel", "OpponentGraveyardPanel"]) as WizardMatchPileView
@onready var local_deck_pile := _resolve_ui_node(["HudLayer/PlayerLibraryPanel", "%PlayerLibraryPanel", "PlayerLibraryPanel"]) as WizardMatchPileView
@onready var local_graveyard_pile := _resolve_ui_node(["HudLayer/PlayerGraveyardPanel", "%PlayerGraveyardPanel", "PlayerGraveyardPanel"]) as WizardMatchPileView
@onready var environment_zone_panel := _resolve_ui_node(["HudLayer/EnvironmentZonePanel", "%EnvironmentZonePanel", "EnvironmentZonePanel"]) as WizardMatchPublicZonePanel
@onready var artifacts_zone_panel := _resolve_ui_node(["HudLayer/ArtifactsZonePanel", "%ArtifactsZonePanel", "ArtifactsZonePanel"]) as WizardMatchPublicZonePanel
@onready var traps_zone_panel := _resolve_ui_node(["HudLayer/TrapsZonePanel", "%TrapsZonePanel", "TrapsZonePanel"]) as WizardMatchPublicZonePanel
@onready var captures_zone_panel := _resolve_ui_node(["HudLayer/LocalCapturesZonePanel", "%LocalCapturesZonePanel", "LocalCapturesZonePanel"]) as WizardMatchPublicZonePanel
...
func _input(event: InputEvent) -> void:
	...
	if interaction_controller.is_dragging() and event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT and event.pressed:
		...
		get_viewport().set_input_as_handled()
		return
	if not interaction_controller.is_dragging() and event is InputEventKey and event.pressed and event.keycode == KEY_ESCAPE and _has_pinned_selection():
		_clear_selection()
		get_viewport().set_input_as_handled()
		return
	if not interaction_controller.is_dragging() and event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT and event.pressed and _has_pinned_selection():
		_clear_selection()
		get_viewport().set_input_as_handled()
		return
...
func _configure_static_ui() -> void:
	...
	hud_layout.configure_nodes({
		...
		"opponent_library_panel": opponent_deck_pile,
		"opponent_graveyard_panel": opponent_graveyard_pile,
		"player_library_panel": local_deck_pile,
		"player_graveyard_panel": local_graveyard_pile,
		"turn_panel": turn_action_panel,
		"match_sidebar": match_sidebar,
		"inspect_popup": inspect_popup,
		"local_hand_row": local_hand_row,
		"opponent_hand_row": opponent_hand_row,
		"inspect_avoid_panels": [
			opponent_deck_pile,
			opponent_graveyard_pile,
			local_deck_pile,
			local_graveyard_pile,
			environment_zone_panel,
			artifacts_zone_panel,
			traps_zone_panel,
			captures_zone_panel,
			turn_action_panel,
		],
	})
	white_portrait_texture = local_status_view.portrait_texture
	black_portrait_texture = opponent_status_view.portrait_texture
...
func _refresh_player_zones() -> void:
	...
	_refresh_status_portraits(display_color)
...
func _refresh_status_portraits(display_color: String) -> void:
	if display_color == ChessEngine.BLACK:
		local_status_view.portrait_texture = black_portrait_texture
		opponent_status_view.portrait_texture = white_portrait_texture
		return
	local_status_view.portrait_texture = white_portrait_texture
	opponent_status_view.portrait_texture = black_portrait_texture
...
func _refresh_inspector() -> void:
	var inspected_card_instance_id := _active_inspected_card_instance_id()
	if not inspected_card_instance_id.is_empty():
		var entry := _find_card_entry(inspected_card_instance_id)
		if not entry.is_empty():
			_set_card_inspector(entry["card_state"], str(entry["zone"]), str(entry["owner_color"]))
			return
	var inspected_square = _active_inspected_square()
	if inspected_square != null:
		_set_square_inspector(inspected_square)
		return
...
func _active_inspected_card_instance_id() -> String:
	if hovered_card_widget != null and is_instance_valid(hovered_card_widget):
		return str(hovered_card_widget.card_state.get("instance_id", ""))
	return selected_card_instance_id


func _active_inspected_square():
	if hovered_square != null and _square_has_inspectable_content(hovered_square):
...
func _on_board_square_pressed(square: Vector2i) -> void:
	if selected_card_instance_id.is_empty() and selected_square == square:
		_clear_selection()
		return
...
func _zone_anchor_global(entry: Dictionary) -> Vector2:
	...
	match zone:
		"deck":
			return (opponent_deck_pile if owner_color == _opponent(_display_color()) else local_deck_pile).get_global_rect().get_center()
		"graveyard":
			return (opponent_graveyard_pile if owner_color == _opponent(_display_color()) else local_graveyard_pile).get_global_rect().get_center()
		"hand":
...
func _square_color(square: Vector2i) -> Color:
	...
	if hovered_square != null and square == hovered_square:
		return base_color.lerp(HOVERED_SQUARE_TINT, 0.30)
	for move_value in selected_moves:
...
func _clear_selection(clear_hover_square: bool = false) -> void:
	selected_card_instance_id = ""
	selected_square = null
	selected_moves.clear()
	selected_hand_card_ids.clear()
	if clear_hover_square:
		hovered_square = null
	_refresh_ui()


func _has_pinned_selection() -> bool:
	return not selected_card_instance_id.is_empty() or selected_square != null or not selected_moves.is_empty() or not selected_hand_card_ids.is_empty()
...
func _square_has_inspectable_content(square: Vector2i) -> bool:
	return wizard_match.chess_engine.get_piece(square) != null or not _attached_cards_for_square(square).is_empty()
